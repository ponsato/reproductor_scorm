...
M.mod_scorm.init = function(Y, nav_display, navposition_left, navposition_top, hide_toc, collapsetocwinsize, toc_title, window_name, launch_sco, scoes_nav) {
    ...
    Y.use('button', 'dd-plugin', 'panel', 'resize', 'gallery-sm-treeview', function(Y) {
        ...
        var scorm_activate_item = function(node) {
            ...
            obj.setAttribute('onload', 'arregla()');
            if (!window_name && node.title != null) {
                obj.setAttribute('src', url_prefix + node.title);
            }
            if (window_name) {
                var mine = window.open('','','width=1,height=1,left=0,top=0,scrollbars=no,status=no,directories=no,menubar=no,toolbar=no,location=no,titlebar=no');
                if(! mine) {
        ...
        var scorm_toggle_toc = function(windowresize) {
            ...
            if (windowresize === true) {
                ...
                if (body.get('winWidth') < collapsetocwinsize) {
                    ...
                    /* JM Contenido del botón para mostrar el índice de contenidos */
                    //scorm_toc_toggle_btn.setHTML('&gt;')
                      /*scorm_toc_toggle_btn.setHTML('Índice de contenidos')
            ...
            /* JM Funcionalidad de mostrar y ocultar tabla TOC */
            if (toc_disabled) {
                toc.removeClass(cssclasses.disabled)
                    .removeAttribute('disabled-by');
                //Para que no se cambie el contenido del botón que habilita el TOC
                /*scorm_toc_toggle_btn.setHTML('&lt;')
                    .set('title', M.util.get_string('hide', 'moodle'));*/ 
                scorm_content.removeClass(cssclasses.scorm_grid_content_toc_hidden)
                    //.addClass(cssclasses.scorm_grid_content_toc_visible);
            } else {
                toc//.addClass(cssclasses.disabled)  JM Desconecto la clase disabled
        ...
        var scorm_resize_layout = function() {
            ...
            // JM Comento el cálculo de la anchura, ya que ahora se utiliza Bootstrap, aquí y en siguiente if
            ...
            newheight = /*Y.one('body').get('winHeight') - 5*/ $(window).height() -60;
            newheight_toc = newheight;
            /*if (newheight < 600) {
                newheight = 600;
            }*/
            Y.one('#scorm_layout').setStyle('height', newheight);
            Y.one('#scorm_toc').setStyle('height', newheight_toc);
        ...
        var cssclasses = {
                ...
                // JM Comento la siguiente clase para que no se pueda restaurar automáticamente
        ...
        if (scorm_disable_toc) {
            ...
            Y.one('#scorm_toc_toggle_btn')
                // JM cambia el contenido del botón
                //.setHTML('&lt;')
            //    .setHTML('Índice de contenidos')
        ...
        if (!scorm_disable_toc) {
            if (scorm_hide_toc == true) {
                ...
                Y.one('#scorm_toc_toggle_btn')
                    // JM cambia el contenido del botón
                    //.setHTML('&gt;')
                    .setHTML('<span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span>')
...
/* JM Nueva funcionalidad de la tabla de contenidos */
var indice_contenidos = 1;
var anchura_ventana; 
var estado_tabla_contenidos;
// Comentar "require" para versiones de Moodle inferiores a 2.9
require(['jquery'], function($) {
// Funcionalidad de mostrar-ocultar tabla de contenidos
    $('#scorm_toc').css({'left':'0px'});
    $('#scorm_toc_toggle_btn').on('click', function() {
        if ($('#scorm_toc_toggle_btn').hasClass('habilita_toggle')){
            indice_contenidos++;
            $('#scorm_toc_toggle_btn').css({'pointer-events':'none'});
            if ((indice_contenidos % 2) == 0) {
                $('#scorm_toc').stop(true,false);
                $('#scorm_toc').animate({
                    left: parseInt($("#scorm_toc").css("left"))== 0?"-="+$("#scorm_toc").outerWidth():0
                }, 1000, function() {
                    $('#scorm_toc_toggle_btn').css({'pointer-events':'auto'});
                });
                $('#scorm_content').animate({
                     right: '16.66666667%'
                }, 1000, function(){
                    $('#scorm_toc').addClass('col-lg-2 col-md-2').css({'display':'none','position':'absolute'});
                    $('#scorm_content').removeClass('col-lg-10 col-md-10 ').addClass('col-lg-12 col-md-12').css({'right':''});
                });
            } else {
                $('#scorm_toc').stop(true,false);
                $('#scorm_toc').css({'display':'inline-block'});
                $('#scorm_toc').animate({
                    left: '0px',
                }, 1000, function(){
                    $('#scorm_toc_toggle_btn').css({'pointer-events':'auto'});
                ...
                $('#scorm_content').animate({
                    ...
                    $('#scorm_toc').addClass('col-lg-2 col-md-2').css({'position':'relative'});
                    $('#scorm_content').removeClass('col-lg-12 col-md-12').addClass('col-lg-10 col-md-10').css({'left':'','right':''});
                });
            }
        }
    });
...
// Mostrar botón de menú según resolución
    $(document).ready(function (){
        $('body').attr('onResize','comprueba_resolu($);')
        anchura_ventana = $(window).width();
        comprueba_resolu($);
        //estado_tabla_contenidos = $('#scorm_toc').css('display');
...
function comprueba_resolu (_this) {
    if (anchura_ventana<=1024) {
        _this('#barra_navegacion').addClass('navbar navbar-default').css({'background':'transparent','margin-bottom':'0'});
        _this('#scorm_toc_toggle').removeClass('pull-left').addClass('pull-right').attr('role','navigation');
        _this('#scorm_toc_toggle_btn').html('<span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span>').attr('data-toggle','collapse').attr('data-target','#scorm_toc').removeClass('habilita_toggle');
        _this('#scorm_toc').removeClass('col-lg-2 col-md-2').addClass('toc_derecha collapse').css({'left':'','display':''});
        _this('.yui3-treeview-children').addClass('dropdown-menu');
        //indice_contenidos = 1;
    } else {
        _this('#barra_navegacion').removeClass('navbar navbar-default');
        _this('#scorm_toc_toggle').removeClass('pull-right').addClass('pull-left');+
        _this('#scorm_toc_toggle_btn').html('Índice de contenidos').addClass('habilita_toggle').removeAttr('data-toggle').removeAttr('data-target');
        _this('#scorm_toc').removeClass('toc_derecha collapse').addClass('col-lg-2 col-md-2').css({'left':'0px','overflow':'auto'});
        if(_this('#scorm_toc').css('display') != 'none'){
            _this('#scorm_content').removeClass('col-lg-12 col-md-12').addClass('col-lg-10 col-md-10').css({'right':''});
            indice_contenidos = 1;
            _this('#scorm_toc').css({'position':'relative'});
        }
        _this('.yui3-treeview-children').removeClass('dropdown-menu');
        //indice_contenidos = 1;
        _this('#scorm_toc').addClass('col-lg-2 col-md-2');
    } 
}

function arregla(){
    $('#scorm_object').contents().find('body').css({'height':'100%','margin':'0','overflow':'hidden'});
    $('#scorm_object').contents().find('#marcocontenido').css({'height':'100%'});
}